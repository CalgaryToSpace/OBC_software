...
//  const char * buf = "main while loop";
  while (1)
  {
//	HAL_GPIO_TogglePin(GPIOE, GPIO_PIN_2);
//	HAL_UART_Transmit(&hlpuart1, (uint8_t*)buf, sizeof(buf), 1000);
//	HAL_UART_Transmit(&hlpuart1, (uint8_t*)"hi", sizeof(2), HAL_MAX_DELAY);
...
{
  /* USER CODE BEGIN 5 */
	const uint8_t * mpiBuf = "MPI task";
  /* Infinite loop */
  for(;;)
  {
	HAL_UART_Transmit(&hlpuart1,(uint8_t*) mpiBuf, sizeof(mpiBuf), 1000);
    osDelay(1);
  }
...
void Error_Handler(void)
{
  ...
  __disable_irq();
  const uint8_t * mpiBuf = "Errorinator";
  while (1)
  {
	  HAL_UART_Transmit(&hlpuart1,(uint8_t*) mpiBuf, sizeof(mpiBuf), 1000);
  }
